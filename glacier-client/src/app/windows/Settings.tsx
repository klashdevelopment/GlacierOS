"use client";
import { Button, Checkbox, Dropdown, Input, Label, Option, OptionGroup, OptionOnSelectData, Persona, SelectionEvents, Slider, Textarea } from "@fluentui/react-components";
import Window from "../components/Window";
import './settings.css';
import { useEffect, useState } from "react";
import { EyeOffFilled, EyeFilled, ArrowUploadFilled, StoreMicrosoft20Filled, Code20Filled, Tv20Filled, HeadsetVr20Filled, Globe20Filled, FoodApple20Filled, Tablet20Filled } from "@fluentui/react-icons";
import Emoji from "../components/Emoji";
import xor from "../utils/XOR";
import constants from "../Constants";
import Icons from "./MoreIcons";
import { computeTheme, CustomTheme, importTheme } from "./ThemeMaker";
export let wallpapers = ["Motion Layered", "Sphere Green", "Sphere Purple", "Light Wave", "Light Fluent", "Dark Fluent", "Light Sand", "Windows Kali", "Motion Cones", "Motion Ribbon", "Motion Blobs", "Sun Daytime", "Sun Mountain", "Sun Setting", "Sun Desert", "Fluent Blue", "Fluent Green", "Fluent Pink", "Fluent Gray", "Windows 11 Dark", "Windows 11 Light", "Sun Nighttime", "MacOS Mojave", "MacOS Catalina", "MacOS Big Sur", "MacOS Monterey", "Chrome OS Default", "Frosty Glaciers", "Ubuntu 20.04", "Ubuntu Clean", "Cyberrush Rooftop (Lux)", "Cyberrush Track (Lux)", "Cyberrush Void (Lux)", "Cyberrush Building (Lux)", "Kali Trail", "Kali Prompt", "Kali Legacy", "Kali Layers", "Kali Luminara", "MacOS Somona", "Glacier Blue", "Raspberry Pi Default", "MacOS Tahoe Dark", "MacOS Tahoe Light", "Windows 10 Beach", "Windows 10 Light", "Windows 10 Cove", "Windows 10 Dark", "visionOS Room (Dark)", "visionOS Room (Light)", "iPadOS 26 Light", "iPadOS 18 Purple", "iPadOS 18 Blue", "MacOS High Sierra", "MacOS Sierra"];
...
export function SetWallpaperNum(wp: string) {
    ...
    if (document.querySelector('.tvOS-header-inner')) {
        (document.querySelector('.tvOS-header-inner') as HTMLDivElement).style.backgroundImage = `url("/windows/wallpaper${wp}.webp")`;
    }
}
export function SetWallpaperURL(wp: string) {
    ...
    if (document.querySelector('.tvOS-header-inner')) {
        (document.querySelector('.tvOS-header-inner') as HTMLDivElement).style.backgroundImage = `url("${wp}")`;
    }
}

export function SetTheme(theme: CustomTheme, taskbar: (os?: string | undefined, customIcons?: Record<string, string> | undefined) => void) {
    document.querySelector('#oscss')?.remove();
    document.querySelectorAll('.creator-css,.customtheme').forEach(e => e.remove());
    var style = document.createElement('style');
    style.className = 'customtheme';
    style.innerHTML = computeTheme(theme);
    document.head.appendChild(style);
    taskbar(theme.icons.base, theme.icons);
}

function Personalization({ switchPage, taskbar }: { switchPage: any,
    taskbar: (os?: string | undefined, customIcons?: Record<string, string> | undefined) => void
}) {
    ...
    async function fileUploadTheme() {
        var theme = await importTheme();
        if(theme) {
            SetTheme(theme, taskbar);
            window.localStorage.setItem('custom-theme', JSON.stringify(theme));
        } else {
    ...
    return (
        <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                ...
                <div style={{ display: 'flex', gap: '5px' }}>
                    <Button appearance="outline" onClick={fileUploadTheme} icon={<ArrowUploadFilled />}></Button>
                    <Dropdown placeholder="Choose a new OS..." style={{ width: '50%' }} onOptionSelect={(event, data) => {
                        ...
                        <OptionGroup label="Windows (Window-based)">
                            <Option value={'windows'} text={"Windows 11"}><StoreMicrosoft20Filled />Windows 11</Option>
                            <Option value={'windows10'} text={"Windows 10"}><StoreMicrosoft20Filled />Windows 10</Option>
                            <Option value={'windowsxp'} text={"Windows XP"} disabled><StoreMicrosoft20Filled />Windows XP</Option>
                        </OptionGroup>
                        <OptionGroup label="MacOS (Window-based)">
                            <Option value={'macos'} text={"MacOS Ventura"}><FoodApple20Filled />MacOS Ventura</Option>
                            <Option value={'macos-high-sierra'} text={"MacOS High Sierra"}><FoodApple20Filled />MacOS High Sierra</Option>
                            <Option value={'tahoe'} text={"MacOS Tahoe (Glass)"}><FoodApple20Filled />MacOS Tahoe (Glass)</Option>
                            <Option value={'tahoe-dark'} text={"MacOS Tahoe (Dark)"}><FoodApple20Filled />MacOS Tahoe (Dark)</Option>
                        </OptionGroup>
                        <OptionGroup label="Other (Window-based)">
                            <Option value={'chromeos'} text={"Chrome OS"}><Globe20Filled />Chrome OS</Option>
                            <Option value={'ubuntu'} text={"Ubuntu Linux"}><Code20Filled />Ubuntu Linux</Option>
                            <Option value={'kali'} text={"Kali Linux"}><Code20Filled />Kali Linux</Option>
                        </OptionGroup>
                        <OptionGroup label="Non window-based">
                            <Option value={'tvos'} text={"tvOS"}><Tv20Filled /> tvOS</Option>
                            <Option value={'tvos-ipados'} text={"iPadOS"}><Tablet20Filled /> iPadOS</Option>
                            <Option value={'tvos-visionos'} text={"visionOS"}><HeadsetVr20Filled />visionOS</Option>
                        </OptionGroup>
                    </Dropdown>
                </div>
            </div>
            <br /><br />
            <b>tvOS Specific Settings</b>

            <div style={{ display: 'flex', gap: '5px' }}>
                <Dropdown onOptionSelect={(ev, dt) => {
                    SetWallpaperURL(dt.optionValue as string);
                }} placeholder="Choose from extra wallpapers" style={{ width: '50%' }}>
                    {[[
                        "https://baylorlariat.com/wp-content/uploads/2022/04/Severance-scaled.jpg",
                        "Severance"
                    ],
                    ["https://www.apple.com/tv-pr/shows-and-films/s/silo/images/season-02/show-home-graphic-header/key-art-01/4x1/Apple_TV_Silo_key_art_graphic_header_4_1_show_home.jpg.og.jpg?1730150727747",
                        "Silo"],
                    ["https://is1-ssl.mzstatic.com/image/thumb/ageP1PYyLi7UlNiWMva32Q/1200x675.jpg",
                        "Ted Lasso"],
                    ].map(([url, name], index) => (
                        <Option key={index} value={url} text={name}>
                            <img src={url} alt={name} style={{ width: '80px', height: '50px', borderRadius: '8px' }} />
                            {name}
                        </Option>
                    ))}
                </Dropdown></div>
        ...
        <Window title="Settings" id="settings" taskbarIconID="settings" color={'gray'} seperateBorder="1px solid #cccccc0a">
            <div className="window-padding">
                <div className="settings-split">
                    ...
                    <div className="right">
                        {tab === 'personalization' && <Personalization taskbar={taskbar} switchPage={setTab} />}
